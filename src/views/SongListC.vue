<template>
  <div class="songList_body">
    <div class="song_top" @click.stop="back" ref="header">
      <span class="iconfont icon-right">歌手</span>
    </div>
    <div class="menu_wrapper" ref="isMenu">
      <div class="song_menu">
        ...
        <div v-else class="song_layput">
          <div class="top_img">
            <img v-lazy="songList.artist.img1v1Url" alt />
            <div class="filter"></div>
            <div class="text">
              <h2>{{songList.artist.briefDesc}}</h2>
          ...
          <div class="song_list_layout">
            <div class="song_list">
              <div class="sequence-play">
                ...
                <span>({{songList.hotSongs&&songList.hotSongs.length}}首)</span>
              </div>
              <div class="song_list_wrapper">
                <ul>
                  <li
                    v-for="(item,index) in songList.hotSongs"
                    :key="index"
                    class="lists"
                    ref="aLi"
  ...
  mounted() {
    this.axios.get("/artists?id=" + `${this.$route.params.id}`).then(res => {
      if (res.data.code == 200) {
        ...
        this.songIds = res.data.hotSongs;
        setTimeout(() => {
          this.show = false;
        }, 1000);
        this.$nextTick(() => {
