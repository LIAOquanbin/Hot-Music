<template>
  <div class="songList_body animation">
    <div class="song_top" @click.stop="back" ref="header">
      <span class="iconfont icon-right">榜单</span>
    </div>
...
export default {
  ...
  components: {},
  mounted() {
    this.axios.get("/top/list?idx=" + `${this.$route.params.id}`).then(res => {
      if (res.data.code == 200) {
        var data = res.data.playlist;
        this.songList = data;
        this.songIds = res.data.playlist.tracks;
